...
const Practice = () => {
	const { currentLesson, kanjiPracticeArray } = useKanjiStore((state) => state);
	const { kanji } = useParams();
	const [currentIndex, setCurrentIndex] = useState(0);
	const [currentAnswerIndex, setCurrentAnswerIndex] = useState<null | number>(
		null,
	);
	const [currentAnswer, setCurrentAnswer] = useState<null | CompleteKanji>(
		null,
	);
	...
	useEffect(() => {
		...
		const filteredKanjis = kanjiPracticeArray.filter(
			(answer) => answer.kanji !== kanjiPracticeArray[currentIndex].kanji,
		);
		const uniqueKanjis = Array.from(
			new Map(filteredKanjis.map((k) => [k.kanji, k])).values(),
		);
		const reducedKanjis = uniqueKanjis.slice(0, 3);
		const shuffledAnswers = shuffleKanjis([
			kanjiPracticeArray[currentIndex],
			...reducedKanjis,
		]);
	...
	const handleNextKanji = () => {
		setCurrentIndex((prev): number => {
			if (!kanjiPracticeArray || prev === kanjiPracticeArray.length - 1)
				return prev;
	...
	return (
		<div className="flex flex-col items-center gap-4 p-3 sm:p-4">
			...
			<h2 className="text-lg sm:text-2xl font-medium text-slate-600">
				JLPT {kanji} Kanji practice
			</h2>
						...
						<div className="text-lg text-gray-700 flex flex-col sm:flex-row sm:gap-2 sm:items-center mb-1">
							...
							<p className="text-center text-2xl">
								{questionType === "onyomi" && !checked
									? "????"
									: kanjiPracticeArray[currentIndex].wk_readings_on
						...
						<div className="text-lg text-gray-700 flex flex-col sm:flex-row sm:gap-2 sm:items-center">
							...
							<p className="text-center text-2xl flex items-center justify-center">
								{questionType === "kunyomi" && !checked ? (
									"????"
								) : kanjiPracticeArray[currentIndex].wk_readings_kun?.length ===
									0 ? (
									<p className="text-lg text-gray-500">none</p>
								) : (
									kanjiPracticeArray[currentIndex].wk_readings_kun
										?.map((word) => word.replace(/^!/, ""))
										.join(" | ")
								)}
							</p>
								...
								<p className="text-base sm:text-lg ">
									{questionType === "kunyomi"
										? kanji.wk_readings_kun?.length === 0
											? "none"
											: kanji.wk_readings_kun
													?.map((word) => word.replace(/^!/, ""))
													.join(" | ")
										: questionType === "onyomi"
											? kanji.wk_readings_on
													?.map((word) => word.replace(/^!/, ""))
													.join(" | ")
											: questionType === "kanji"
												? kanji.kanji
