...
const Test = () => {
  const {kanjiLevel } = useKanjiStore(state => state)
  ...
  const shuffleAnswers = () =>{
    const newArray = Array.from({length: 20}, ()=>{
      const randomIndex = Math.floor(Math.random() * kanjiLessons.length)
      return kanjiLessons[randomIndex]
    })
    const finalArray = [...newArray].sort(()=> Math.random() - 0.5 )
    return finalArray
  }
